...
class Settings(BaseModel):
    ...
    anthropic_max_tokens: int = Field(
        default_factory=lambda: int(os.environ.get("ANTHROPIC_MAX_TOKENS", "8000"))
    )

    # ElevenLabs
    eleven_api_key: str = Field(
        default_factory=lambda: os.environ.get("ELEVEN_API_KEY", "")
    )
    eleven_voice_id: str = Field(
        default_factory=lambda: os.environ.get("ELEVEN_VOICE_ID", "")
    )
    ...
    r2_endpoint: str = Field(
        default_factory=lambda: os.environ.get("R2_ENDPOINT", "")
    )
    ...
    r2_public_url: str = Field(
        default_factory=lambda: os.environ.get("R2_PUBLIC_URL", "")
    )
    r2_bucket: str = Field(
        default_factory=lambda: os.environ.get("R2_BUCKET", "autopodcast")
    )
